...
class JointedTurtle: public Turtle
{
private:
	...
	vector<vector<vmath::Vector4d>> hemisphere;
	...
	bool sphereTriangulation(bool up);
	...
	void createHemisphere(unsigned int detail);

public:
	JointedTurtle(double thickness, double angle, double length, unsigned int detail)
	{
		...
		createHemisphere(detail);
		loadColors();
	};
	JointedTurtle(Turtle* turtle, unsigned int detail)
	{
	...
	JointedTurtle(const JointedTurtle &copy);
	JointedTurtle operator=(const JointedTurtle &copy);
	...
	inline vector<vector<vmath::Vector4d>> getHemisphere() const { return hemisphere;};
};
